...
const App: React.FC = () => {
  ...
  const handleExport = () => {
    // Extract info from filename (simple assumption) or just use filename
    const report = `=== 3D动画评分报告 ===
文件名称: ${fileName}
评分时间: ${new Date().toLocaleDateString()}
总分: ${totalScore} / ${MAX_TOTAL_SCORE}

${RUBRIC_DATA.map(item => `[${item.category}] ${scores[item.id]}/${item.maxScore}\n评语: ${comments[item.id] || '无'}`).join('\n\n')}
`;
